...
def get_system_stat(stframe1, stframe2, stframe3, fps, df_fq):
    ...
    with stframe3.container():
        ...
        # Updating System stats with specific pid
        # with js1:
        #     st.markdown("<h4>Memory usage</h4>", unsafe_allow_html=True)
        #     pid = 7656 # replace with your process ID
        #     mem_use = psutil.Process(pid).memory_percent()
        #     if mem_use > 20:
        #         js1_text = st.markdown(
        #             f"<h5 style='color:red;'>{round(mem_use, 2)}%</h5>", unsafe_allow_html=True)
        #     else:
        #         js1_text = st.markdown(
        #             f"<h5 style='color:green;'>{round(mem_use, 2)}%</h5>", unsafe_allow_html=True)
